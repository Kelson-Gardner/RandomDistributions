...
Bins generateBins(const std::uint32_t min, const std::uint32_t max, const
...
{
const auto add_counts = [](const std::uint32_t total, const DistributionPair&
bin)
{
return total + bin.count;
};
const auto actual = std::accumulate(bins.cbegin(), bins.cend(), 0u,
add_counts);
